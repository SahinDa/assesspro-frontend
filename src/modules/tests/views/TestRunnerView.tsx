...
export interface TestRunnerProps {
  ...
  onSubmitAttempt?: (payload: {
    ...
    violations: Array<{ type: ViolationType; timestamp: number }>
  }) => void
}

// Fallback Mock Questions for UI Simulation
...
export default function TestRunnerView({
  ...
  // References for periodic 1-minute delta sync
  const dirtyAnswersRef = useRef<Record<string, number>>({})
  const totalSecondsRef = useRef(timerMinutes * 60)

  // ---------------------------------------------------------------------------
  // INITIALIZE TEST (API Call Placeholder)
  // ---------------------------------------------------------------------------
  useEffect(() => {
    const initializeExam = async () => {
      setIsInitializing(true)
      try {
        // UI simulation: replace with `const res = await api.post('/tests/start', { testSetId })`
  ...
  const recordViolation = useCallback(
    (type: ViolationType, message: string) => {
      ...
      setLatestWarning(`Proctoring Warning: ${message}`)

      // API trigger placeholder for violation sync
      console.log(`[API Call: POST /proctor/violation]`, entry)
    },
    [isSubmitted, isPreview]
  )

  // ---------------------------------------------------------------------------
  // SECURITY LISTENERS (Tab switch, window blur, fullscreen, copy/paste, shortcuts)
  // ---------------------------------------------------------------------------
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        recordViolation(ViolationType.TAB_SWITCH, 'Tab switch or window minimized detected.')
      }
    }

    const handleWindowBlur = () => {
      recordViolation(ViolationType.WINDOW_BLUR, 'Focus lost from assessment window.')
    }
    ...
    const handleFullscreenChange = () => {
      ...
      if (!inFullscreen) {
        recordViolation(ViolationType.FULLSCREEN_EXIT, 'Fullscreen mode exited.')
      }
    }

    const handleCopy = (e: ClipboardEvent) => {
      e.preventDefault()
      recordViolation(ViolationType.CLIPBOARD_COPY, 'Copying exam content is disabled.')
    }

    const handlePaste = (e: ClipboardEvent) => {
      e.preventDefault()
      recordViolation(ViolationType.CLIPBOARD_PASTE, 'Pasting external content is disabled.')
    }

    const handleKeyDown = (e: KeyboardEvent) => {
      // F12 or Ctrl+Shift+I / Cmd+Option+I (Inspect)
      if (
        e.key === 'F12' ||
        ((e.ctrlKey || e.metaKey) && e.shiftKey && (e.key === 'I' || e.key === 'i' || e.key === 'J' || e.key === 'j'))
      ) {
        e.preventDefault()
        recordViolation(ViolationType.DEV_TOOLS, 'Developer tools access attempt detected.')
      }
    }

    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      // API trigger placeholder on page unload / window close
      navigator.sendBeacon?.(
        '/api/test-attempts/heartbeat',
        JSON.stringify({ testSetId, status: 'WINDOW_CLOSED', timestamp: Date.now() })
      )
      e.preventDefault()
      e.returnValue = ''
    }
  ...
  }, [isInitializing, isSubmitted, recordViolation, testSetId])

  // ---------------------------------------------------------------------------
  // 1-MINUTE PERIODIC DELTA ANSWER SYNC
  // ---------------------------------------------------------------------------
  useEffect(() => {
    if (isInitializing || isSubmitted) return
    ...
    const syncInterval = setInterval(() => {
      if (Object.keys(dirtyAnswersRef.current).length > 0) {
        console.log('[API Call: POST /test-attempts/sync-answers] Batch payload:', dirtyAnswersRef.current)
        // Clear dirty queue once synced
  ...
  const toggleFullscreen = async () => {
    try {
      ...
      // Ignored if browser prohibits programmatic triggers
    }
  }

  // Question selection & option changes
  ...
  const handleFinalSubmit = () => {
    ...
    console.log('[API Call: POST /test-attempts/submit] Result:', payload)
  }
    ...
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center space-y-3">
        ...
        <div className="text-center">
          <p className="text-sm font-bold text-slate-800">Initializing Exam Session...</p>
          <p className="text-xs text-slate-500">Configuring environment and anti-cheat modules.</p>
        </div>
      </div>
    )
  }
  ...
  // ---------------------------------------------------------------------------
  // 1. POST-EXAM RESULTS BREAKDOWN (Upon Submission)
  // ---------------------------------------------------------------------------
  if (isSubmitted) {
    let correctCount = 0
          ...
          {violations.length > 0 && (
            <div className="p-3.5 rounded-2xl bg-amber-50 border border-amber-200 text-left text-xs text-amber-900 space-y-1">
              <div className="flex items-center gap-1.5 font-bold text-amber-950">
                ...
                <span>{violations.length} Proctoring Event(s) Flagged</span>
              </div>
              <p className="text-[11px] text-amber-800">
                Suspicious tab changes or focus losses were logged and sent with your submission payload.
              </p>
            </div>
          )}
  ...
  // ---------------------------------------------------------------------------
  // 2. ACTIVE EXAM RUNNER WORKSPACE
  // ---------------------------------------------------------------------------
  const optionKeys = [
  ...
  return (
    <div className="space-y-4 max-w-7xl mx-auto select-none">
      ...
      {/* Real-time Proctoring Toast Warning */}
      {latestWarning && (
        <div className="p-3 rounded-2xl bg-rose-50 border border-rose-200 flex items-center justify-between gap-3 text-rose-800 text-xs">
          <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="text-xs text-rose-600 hover:text-rose-900 font-bold cursor-pointer"
          >
            Dismiss
          </button>
        </div>
      ...
      {/* Main Runner Body: Split into Question Area & Question Palette */}
